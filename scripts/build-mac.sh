#!/usr/bin/env bash
# ── Build script for macOS (run from project root) ──
# Prereqs: Node.js 18+, npm, Xcode command line tools
set -euo pipefail
...
echo "═══════════════════════════════════════════════"
echo "  Unweave — macOS Build Pipeline"
echo "═══════════════════════════════════════════════"
...
if [ ! -d "$PYTHON_DIR" ]; then
    echo ""
    echo "📦 Step 2/5: Downloading standalone Python ${PYTHON_VERSION} (${PYTHON_ARCH})..."
...
# Install macOS PyTorch (MPS built-in for Apple Silicon) + base deps
...
echo "   ✅ Base dependencies installed (PyTorch with MPS support)"
